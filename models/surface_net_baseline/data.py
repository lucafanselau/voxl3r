...
class OccSurfaceNetDataset(Dataset):
  ...
  def __getitem__(self, idx):
    ...
    images, transformations, points, gt = SceneDatasetTransformToTorch("mps").forward(self.chunks[idx])
    images = images / 255.0
    
    if self.p_batch_size is not None:
      indices = np.random.choice(points.shape[0], self.p_batch_size, replace=False)
      points = points[indices]
      gt = gt[indices]

    X = project_points_to_images(points, images, transformations)
    # Initialize a new tensor of shape [128, 30], filled with -1s
    X_extended = -1 * torch.ones(X.shape[0], 3*self.dataset.max_seq_len).to(X.device)
...
def custom_collate_fn(batch):
  ...
  return batch_stacked[:, :-4], batch_stacked[:, -4].unsqueeze(1), batch_stacked[:, -3:]

class OccSurfaceNetDatamodule(pl.LightningDataModule):
  def __init__(self, dataset: SceneDataset, scene_id: str, batch_size = 512, max_sequence_length = 20, target_device="mps", single_chunk=True):
    super().__init__()
    ...
    if single_chunk:
      ...
      if isinstance(scene_id, list) and len(scene_id) == 3:
        self.scene_idx_train, self.scene_idx_val, self.scene_idx_test  = dataset.get_index_from_scene(scene_id)
        ...
        raise ValueError("If single_chunk is False, scene_id should be a list of length 3")
  ...
  def setup(self, stage):
    if self.single_chunk:
      ...
      generator = torch.Generator().manual_seed(42)
      self.split = torch.utils.data.random_split(self.dataset, [0.6, 0.2, 0.2], generator=generator)
  ...
  def train_dataloader(self):
    if self.single_chunk:
      ...
      return torch.utils.data.DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, collate_fn=custom_collate_fn)
  
  def val_dataloader(self):
    if self.single_chunk:
        return torch.utils.data.DataLoader(self.val_dataset, batch_size=self.batch_size, shuffle=True)
    else:
      return torch.utils.data.DataLoader(self.val_dataset, batch_size=self.batch_size, shuffle=True, collate_fn=custom_collate_fn)
  ...
  def test_dataloader(self):
    if self.single_chunk:
      ...
      return torch.utils.data.DataLoader(self.test_dataset, batch_size=self.batch_size, shuffle=True, collate_fn=custom_collate_fn)
