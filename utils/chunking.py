...
def get_images_with_3d_point(points, camera_params, keys = None, tolerance=0.9, max_seq_len=8):
        """
...
def create_chunk(mesh, image_name, camera_params_scene, center=np.array([0.0, 0.0, 1.25]), size=np.array([1.5, 1.0, 2.0]), max_seq_len=8, image_path = None, with_backtransform=True):
    transformation = camera_params_scene[image_name]["T_cw"]
    _, _, back_transformation = invert_pose(transformation[:3,:3], transformation[:3,3])
    ...
    image_names, pixel_coordinate, camera_params_list = (
            get_images_with_3d_point(
                p_center, camera_params_scene, keys = image_names[idx:], tolerance=0.8, max_seq_len=max_seq_len
            )
        )
    
    mesh = chunk_mesh(mesh, transformation, center, size, with_backtransform=with_backtransform)
...
def chunk_mesh(mesh, transformation, center=np.array([0.0, 0.0, 1.25]), size=np.array([1.5, 1.0, 2.0]), with_backtransform=True):    
