...
from .image import (
    Dataset as ImageDataset,
    Config as ImageDatasetConfig,
    Output as ImageOutput,
)
...
class Dataset(ImageDataset):
    ...
    def on_after_prepare(self):
        ...
        if (
            ...
            / f"last_cache_{self.split}.pt"
        ).exists() and self.data_config.load_last_cached:
    ...
    def get_at_idx(self, idx: int, fallback: bool = False) -> dict:
        ...
        if self.file_names is None:
            raise ValueError(
                "No file names loaded. Did you forget to call prepare_data()?"
            )
        ...
        if idx < 0 or idx >= len(all_files):
            raise IndexError(
                f"Index {idx} out of range for dataset length {len(all_files)}."
            )
        ...
        image_paths = chunk_dict.get("image_pairs", [])
        image_paths = [
            str(self.base_dataset.get_image_dir(chunk_dict["scene_name"]) / i)
        ...
        mast3r_size = (512, 336)
        images_tensor = (
            self._load_images_into_tensor(image_paths, size=mast3r_size[0]) / 255.0
        )
    ...
    def _load_images_into_tensor(
        self, image_paths: List[Union[str, Path]], size=512
    ) -> torch.Tensor:
        """
        ...
        for img_path in image_paths:
            ...
            if img_path in self.cache:
                loaded_images.append(
                    self.decompress_png_to_tensor(self.cache[img_path])
                )
                continue

            saving_path = (
                Path(img_path).parents[1]
                / self.data_config.compressed_img_folder
                / Path(img_path).name
            )
