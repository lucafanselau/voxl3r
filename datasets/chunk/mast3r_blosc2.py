...
key2memory = {
    ...
    "desc_conf": 2000,
    "dec_0": 2000,
    **{f"dec_{i}": 2000 for i in range(1, 13)},
}
...
class Dataset(ChunkBaseDataset):
    def __init__(
        ...
        # initialize SChunk storage:

        contiguous = True
        
        data_dir = Path(self.data_config.mast3r_data_dir or self.data_config.data_dir) / self.data_config.storage_preprocessing / self.data_config.data_dir_schunk
                        
        self.schunk = {}
        for key in data_config.mast3r_keys:
            ...
            self.schunk[key] = blosc2.SChunk(
                chunksize=200 * 1000 * 4, cparams=cparams, dparams=dparams, storage=storage
            )
        ...
        for batch_idx, batch in tqdm(
            ...
            try:
                for master_chunk_dict, saving_dir, file_name in self.process_chunk(batch, batch_idx, model):
                    ...
                    if self.config.mast3r_keys is not None:
                        for key in self.config.mast3r_keys:
                            tensor_cat = torch.stack([master_chunk_dict["pairwise_predictions"][0][key], master_chunk_dict["pairwise_predictions"][1][key]])
                            nchunks_ = self.schunk[key].append_data(tensor_cat)
                            if len(self.schunk.vlmeta) == 1:
                                self.schunk.vlmeta["file_names"] = [(file_name, nchunks_)]
                            else:
                                self.schunk.vlmeta["file_names"] = self.schunk.vlmeta["file_names"].append((file_name, nchunks_))
                        
                    else:
                        raise ValueError("No keys provided for saving")
