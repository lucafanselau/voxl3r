...
class Dataset(Dataset):
    def __init__(
        ...
        if data_config.split is None:
            ...
            if data_config.split == "test":
                split_scenes_path = (
                    Path(data_config.data_dir) / "splits" / f"nvs_sem_val.txt"
                )  #  f"sem_{data_config.split}.txt"
            elif data_config.split in ["val"]:
                split_scenes_path = (
                    Path(data_config.data_dir)
                    / "splits"
                    / f"nvs_sem_{data_config.split}.txt"
                )
            ...
            if data_config.scenes is not None:
                ...
                if len(not_in_split) > 0:
                    logger.warning(
                        f"Split {data_config.split} does not contain the scenes: {not_in_split}"
                    )

                self.scenes = list(
                    set.intersection(
                        *[set(ids) for ids in [split_scenes, data_config.scenes]]
                    )
                )
            else:
    ...
    def load_paths(self):
        ...
        for scene_name in self.scenes:
            data_dir = self.get_voxelized_scene_dir(
                scene_name, resolution=self.data_config.scene_resolution
            )
            if data_dir.exists():
    ...
    def check_voxelized_scene_exists(self, scene_name: str) -> bool:
        voxelized_scene_dir = self.get_voxelized_scene_dir(
            scene_name, resolution=self.data_config.scene_resolution
        )
    ...
    def prepare_scene(self, scene_name: str):
        ...
        if (
            self.check_voxelized_scene_exists(scene_name)
            and not self.data_config.force_prepare_voxelize
        ):
            # we need to check if all of the chunks for this scene are present
            logger.trace(
                f"Voxelized scenes for scene {scene_name} already exist. Skipping."
            )
            return

        mesh_path = self.data_dir / scene_name / "scans" / "mesh_aligned_0.05.ply"
        ...
        voxelized_scene = self.voxelize_scene(
            scene_name, resolution=self.data_config.scene_resolution
        )
        ...
        torch.save(
            voxelized_scene, data_dir / self.get_file_name_voxelized_scene(scene_name)
        )
    ...
    def get_voxelized_scene(self, scene_name):
        ...
        return torch.load(
            voxelized_scene_dir / self.get_file_name_voxelized_scene(scene_name),
            weights_only=False,
        )
    ...
    def get_image_dir(self, scene_name):
        image_dir = "rgb" if self.camera == "iphone" else "undistorted_images"
        return self.data_dir / scene_name / self.camera / image_dir
    ...
    def __getitem__(self, idx):
        ...
        if self.camera == "dslr" and not (
            ...
            raise ValueError(
                f"Please run the undistortion script for scene {self.scenes[idx]} first"
            )
        ...
        shared_return = {
            ...
            "mesh": mesh,
            "path_images": self.get_image_dir(self.scenes[idx]),
        ...
        if self.data_config.return_voxelized:
            ...
            return {
                **shared_return,
                "voxelized_scene": self.get_voxelized_scene_dir(
                    self.scenes[idx], resolution=self.data_config.scene_resolution
                ),
            }
    ...
    def find_total_volume(self, target_chunks=10_000):
        ...
        logger.info(
            f"Average volume per chunk: {avg_volume_per_chunk} with target chunks: {target_chunks}"
        )
