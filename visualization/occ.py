...
class Visualizer(base.Visualizer):
    ...
    def add_from_voxel_grid(
        self,
        voxel_grid,
        opacity: Optional[float] = 0.5,
        to_world: Optional[bool] = True,
        transformed=False,
    ) -> None:
    ...
    def add_from_occupancy_dict(
        ...
        self.add_occupancy(
            occupancy.int(),
            torch.Tensor(T_wc) if to_world else None,
            pitch=data["resolution"],
            origin=torch.Tensor(center),
            opacity=opacity,
            further_rotation=further_rotation,
        )

    def add_from_occupancy_dict_as_points(
        self,
        occupancy_dict: dict,
        opacity: Optional[float] = 0.5,
        to_world: Optional[bool] = True,
        ...
        with_transform: Optional[bool] = True,
    ) -> None:

        if with_transform:
            data_dict = occupancy_dict["verbose"]["data_dict"]
            coordinates = occupancy_dict["verbose"]["coordinates"]
            occupancy_grid = rearrange(
                data_dict["occupancy_grid"], "1 X Y Z -> (X Y Z) 1"
            )
        else:
            ...
            occupancy_grid = rearrange(
                occupancy_dict["occupancy_grid"], "1 X Y Z -> (X Y Z) 1"
            )
    ...
    def visualize_batch(
        ...
        origin: Float[np.ndarray, "batch 3"] = np.zeros(3),
        ...
        for i in range(batch_size):
            ...
            grid = self._create_voxel_grid(
                voxels[i], origin=offset, pitch=pitch, further_rotation=further_rotation
            )
            ...
            elif voxels.shape[1] == 4:
                # eg. rgba
                rgba = np.moveaxis(voxels[i], 0, -1)
                rgb = rgba[..., :3]
                alpha = rgba[..., 3]
                if rgb.max() > 1.0:
                    rgb = rgb / 255.0
                grid.cell_data["RGB"] = rgba.reshape(-1, 4, order="F")
                # grid.cell_data["SCALARS"] = alpha.flatten(order="F")
    ...
    def add_from_scene_occ(self, dict):
        ...
        self.visualize_batch(
            torch.ones_like(occupancy).unsqueeze(0),
            mask=occupancy,
            pitch=grid.pitch[0].item(),
            opacity=0.5,
        )
    ...
    def add_occupancy(
        ...
        if occupancy.dtype == torch.float:
            # check if occupancy is already a color voxel grid
            if color is not None:
                voxels = color.unsqueeze(0)
            else:
                voxels = occupancy.unsqueeze(0).repeat(1, 3, 1, 1, 1)
            mask = occupancy.unsqueeze(0)
        else:
            # check if occupancy is already a color voxel grid
            if color is not None:
                voxels = color.unsqueeze(0)
            else:
                voxels = torch.ones_like(occupancy).unsqueeze(0).repeat(1, 3, 1, 1, 1)
            mask = occupancy.unsqueeze(0)
