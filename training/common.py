...
def load_config_from_checkpoint(
    project_name, run_name, checkpoint_name="last", ConfigClass=None
):
...
def create_dataset_rgb(config, split: str, transform=nn.Module):
    ...
    image_dataset = chunk.image_loader_compressed.Dataset(
        config, base_dataset, split=config.split
    )
...
def create_dataset(config, split: str, transform):
    # config.scenes = None
    config.split = split
    if config.split == "test":
        config.split = "val"
    ...
    zip = chunk.zip.ZipChunkDataset(
        [
            ...
            # chunk.mast3r_blosc2.Dataset(config, base_dataset, image_dataset),
        ],
        transform=transform,
    )
...
class DataConfigRGB(
    DataConfig,
    chunk.image_loader.Config,
    transforms.SmearImagesConfig,
    transforms.PairingTransformConfig,
):
    pass


def create_datamodule_rgb(
    config: DataConfigRGB,
    splits=["train", "val", "test"],
    DataModuleClass=DefaultDataModule,
):

    datasets = {
        split: create_dataset_rgb(
            config, split, transform=transforms.ComposeTransforms(config)
        )
        for split in splits
    }
    ...
    for split in splits:
        ...
        collate_fn.transforms = [
            transform(config, None) for transform in collate_fn.transforms
        ]
        collate_fns[split] = collate_fn

    datamodule = DataModuleClass(
        data_config=config, datasets=datasets, collate_fn=collate_fns
    )
    return datamodule


class DataConfigRGBLMDB(
    DataConfig, chunk.image_loader_lmdb.Config, transforms.SmearImagesConfig
):
    pass


def create_datamodule_rgb_lmdb(
    config: DataConfigRGBLMDB,
    splits=["train", "val", "test"],
    DataModuleClass=DefaultDataModule,
    collate_fn=None,
):
    ...
    datasets = {
        split: create_dataset_rgb_lmdb(config, split, transform=transform)
        for split in splits
    }
    datamodule = DataModuleClass(
        data_config=config, datasets=datasets, collate_fn=collate_fn
    )
    return datamodule
...
def create_datamodule(
    ...
    datasets = {
        split: create_dataset(
            config.model_copy(), split, transform=transforms.ComposeTransforms(config)
        )
        for split in splits
    }
...
class DataConfigLMDB(
    DataConfig, chunk.mast3r_lmdb.Config, transforms.SmearMast3rConfig
):
    pass


def create_datamodule_lmdb(
    config: DataConfigLMDB,
    splits=["train", "val", "test"],
    DataModuleClass=DefaultDataModule,
    collate_fn=None,
):
    transform = transforms.ComposeTransforms(config)
    datasets = {
        split: create_dataset_lmdb(config, split, transform=transform)
        for split in splits
    }
    datamodule = DataModuleClass(
        data_config=config, datasets=datasets, collate_fn=collate_fn
