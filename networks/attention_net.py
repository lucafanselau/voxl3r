...
from networks.u_net import Block1x1_3x3
...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, seq_len=None, num_pairs=4, use_abs_pe=False):
        ...
        self.has_flex = False #check_flex_attention()
        if self.has_flex:
            ...
            BLOCK_SIZE = 128
            self.attn_mask = create_block_mask(distance_3d_mask, B=None, H=None, Q_LEN=seq_len, KV_LEN=seq_len, BLOCK_SIZE=BLOCK_SIZE, _compile=True)
...
class AttentionNet(nn.Module):
    def __init__(self, config: AttentionNetConfig):
        ...
        downscaling_convs_factor = 4
        X = X // downscaling_convs_factor
        Y = Y // downscaling_convs_factor
        Z = Z // downscaling_convs_factor
        px, py, pz = (1, 1, 1)
        ...
        # self.downscaling_with_conv = nn.Sequential(
        #     # mapping local feature
        #     nn.InstanceNorm3d(config.per_voxel_channels),
        #     nn.Conv3d(config.per_voxel_channels, config.per_voxel_channels, kernel_size=1),
        #     nn.InstanceNorm3d(config.per_voxel_channels),
        #     nn.GELU(),
        #     nn.Conv3d(config.per_voxel_channels, config.per_voxel_channels, kernel_size=3, padding=1),
        #     nn.InstanceNorm3d(config.per_voxel_channels),
        #     nn.GELU(),
        #     # downscale the data two times
        #     nn.Conv3d(config.per_voxel_channels, self.config.dim, kernel_size=2, stride=2),
        #     nn.InstanceNorm3d(self.config.dim),
        #     nn.GELU(),
        #     nn.Conv3d(config.per_voxel_channels, self.config.dim, kernel_size=2, stride=2),
        #     nn.InstanceNorm3d(self.config.dim),
        #     nn.GELU()
    ...
    def forward(self, previous):
        ...
        x = self.downscaling_with_conv(x)
...
class AttentionNet(nn.Module):
    def __init__(self, config: AttentionNetConfig):
        ...
        num_patches = (X // px) * (Y // py) * (Z // pz) * config.num_pairs
        patch_dim = config.per_voxel_channels * (px * py * pz + 4**(downscaling_convs_factor//2))
        self.patch_dim = patch_dim
        p = config.num_pairs
        ...
        self.downscaling_with_conv = nn.Sequential(
            ...
            nn.GELU(),
            # downscale the data two times
            nn.Conv3d(config.per_voxel_channels, patch_dim//2, kernel_size=2, stride=2),
            nn.InstanceNorm3d(patch_dim//2),
            nn.GELU(),
            nn.Conv3d(patch_dim//2, patch_dim, kernel_size=2, stride=2),
            nn.InstanceNorm3d(patch_dim),
            nn.GELU()
        )
    ...
    def forward(self, previous):
        ...
        x = rearrange(x, "b p c x y z -> (b p) c x y z")
        x = self.downscaling_with_conv(x)
        x = rearrange(x, "(b p) c x y z -> b p c x y z", p=p, b=b)
        x = self.to_patch_embedding(x)
